...
class RetinaNet(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        pred_logits, pred_anchor_deltas, pred_features = self.head(features)
        # Transpose the Hi*Wi*A dimension to the middle:
        pred_logits = [permute_to_N_HWA_K(x, self.num_classes) for x in pred_logits]
        pred_anchor_deltas = [permute_to_N_HWA_K(x, 4) for x in pred_anchor_deltas]
        pred_features = [permute_to_N_HWA_K(x, 256) for x in pred_features] # [N, HW, 256]
        K = self.anchor_generator.num_cell_anchors[0]
        N = pred_features[0].size(0)
        r_pred_features = []
        for pred_f in pred_features:
            pred_f = pred_f.view(N, -1, 1, 256)
            pred_f = pred_f.repeat(1, 1, K, 1)
            pred_f = pred_f.view(N, -1, 256)
            r_pred_features.append(pred_f)
        pred_features = r_pred_features
        ...
        if self.training:
            ...
            results = self.inference(anchors, pred_logits, pred_anchor_deltas, pred_features, images.image_sizes)
            processed_results = []
    ...
    def losses(self, anchors, pred_logits, gt_labels, pred_anchor_deltas, gt_boxes):
        ...
        pos_mask = (gt_labels >= 0) & (gt_labels != (self.num_classes - 1))
    ...
    def label_anchors(self, anchors, gt_instances):
        ...
        for gt_per_image in gt_instances:
            ...
            if len(gt_per_image) > 0:
                ...
                gt_labels_i[anchor_labels == 0] = self.num_classes - 1
                # Anchors with label -1 are ignored.
                gt_labels_i[anchor_labels == -1] = -1
            else:
                matched_gt_boxes_i = torch.zeros_like(anchors.tensor)
                gt_labels_i = torch.zeros_like(matched_idxs) + self.num_classes - 1
    ...
    def inference(self, anchors, pred_logits, pred_anchor_deltas, pred_features, image_sizes):
        """
        Arguments:
            anchors (list[Boxes]): A list of #feature level Boxes.
        ...
        for img_idx, image_size in enumerate(image_sizes):
            ...
            features_per_image = [x[img_idx] for x in pred_features]
            results_per_image = self.inference_single_image(
                anchors, pred_logits_per_image, deltas_per_image, features_per_image, tuple(image_size)
            )
            results.append(results_per_image)
        return results

    def inference_single_image(self, anchors, box_cls, box_delta, features, image_size):
        """
        ...
        for box_cls_i, box_reg_i, anchors_i, features_i in zip(box_cls, box_delta, anchors, features):
            # (HxWxAxK,)
            ...
            cls_features = features_i[anchor_idxs]
            ...
            features_all.append(cls_features)
        ...
        result.cls_features = features_all[keep]
        return result
...
class RetinaNetHead(nn.Module):
    ...
    def forward(self, features):
        ...
        for feature in features:
            cls_f = self.cls_subnet(feature)
            cls_features.append(cls_f)
            logits.append(self.cls_score(cls_f))
            bbox_reg.append(self.bbox_pred(self.bbox_subnet(feature)))
        return logits, bbox_reg, cls_features
